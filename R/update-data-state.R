...
state_filt <- if (length(argv) >= 1) argv[1] else "PB"
states_dir <- if (length(argv) >= 2) argv[2] else "states"

print(paste("Updating data for", state_filt, "in", states_dir))
...
brazil_data <- brazil_data[place_type == "city"][city != "Importados/Indefinidos"][,
                            date := as.Date(date, format = "%Y-%m-%d")][state %in% state_filt]
...
# Save data  --------------------------------------------------------------
file_dir <- here::here(states_dir, state_filt, "data", "cases")
#file_dir <- here::here("data", states_dir, state_filt, "cases")
if (!dir.exists(file_dir)) dir.create(file_dir, recursive = TRUE)
data.table::fwrite(brazil_data, file.path(file_dir, paste0(file_name, ".csv")))
